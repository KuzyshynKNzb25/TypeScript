var findByLegAndHypotenuse = function (leg, hypotenuse) {
    if (leg >= hypotenuse) {
    ...
    console.log("a = ".concat(leg, "\nb = ").concat(leg2, "\nc = ").concat(hypotenuse, "\nalpha = ").concat(alpha, "\nbeta = ").concat(beta, "\n\"success\""));
};
var findByTwoLegs = function (leg1, leg2) {
    ...
    console.log("a = ".concat(leg1, "\nb = ").concat(leg2, "\nc = ").concat(c, "\nalpha = ").concat(alpha, "\nbeta = ").concat(beta, "\n\"success\""));
};
var findByLegAndAdjacentAngle = function (adjacentAngle, Leg) {
    ...
    var in_radians = adjacentAngle * Math.PI / 180;
    var c = Leg / Math.cos(in_radians);
    var Leg2 = Leg * Math.tan(in_radians);
    var beta = 90 - adjacentAngle;
    console.log("a = ".concat(Leg, "\nb = ").concat(Leg2, "\nc = ").concat(c, "\nalpha = ").concat(adjacentAngle, "\nbeta = ").concat(beta, "\n\"success\""));
};
var findByLegAndOppositeAngle = function (oppositeAngle, Leg) {
    ...
    var in_radians = oppositeAngle * Math.PI / 180;
    var beta = 90 - oppositeAngle;
    var c = Leg / Math.sin(in_radians);
    var Leg2 = Leg * Math.tan(beta * Math.PI / 180);
    beta = 90 - oppositeAngle;
    console.log("a = ".concat(Leg, "\nb = ").concat(Leg2, "\nc = ").concat(c, "\nalpha = ").concat(oppositeAngle, "\nbeta = ").concat(beta, "\n\"success\""));
};
var findByHipotenuseAndAngle = function (angle, hypotenuse) {
    if (angle > 89.999) {
        console.log("Значення цього кута повинно бути меншим за 89.999\n" +
            "Value of that angle must be less than 89.999\n\"failed\"");
        return;
...
var availableInput = ["leg", "hypotenuse", "adjacent angle", "opposite angle", "angle"];
var rightInputs = [[0, 1], [2, 0], [3, 0], [4, 1], [0, 0]];
var triangle = function (number1, nameOfThatNumber, number2, nameOfNumber2) {
